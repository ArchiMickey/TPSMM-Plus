...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(
        ...
        if multi_mask:
            ...
            for i in range(self.occlusion_num):
                occlusion.append(
                    nn.Conv2d(channel[i], 1, kernel_size=(3, 3), padding=(1, 1))
                )
            self.occlusion = nn.ModuleList(occlusion)
        else:
            occlusion = [
                nn.Conv2d(
                    hourglass_output_size[-1], 1, kernel_size=(3, 3), padding=(1, 1)
                )
            ]
            self.occlusion = nn.ModuleList(occlusion)
    ...
    def create_deformed_source_image(self, source_image, transformations):
        ...
        use_bf16 = (source_image.dtype == torch.bfloat16) or (
            transformations.dtype == torch.bfloat16
        )
        if use_bf16:
