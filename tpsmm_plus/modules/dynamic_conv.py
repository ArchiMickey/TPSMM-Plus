...
class Dynamic_conv2d(nn.Module):
    ...
    def forward(self, x,y):#将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的
        ...
        y = y.view(1, -1, height, width).contiguous()# 变化成一个维度进行组卷积
        weight = self.weight.view(self.K, -1)
        ...
        output = output.view(batch_size, self.out_planes, output.size(-2), output.size(-1)).contiguous()
        return output

class Dynamic_conv2d_conditional(nn.Module):
    def __init__(self, conditional_dim, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1, bias=True, K=4,temperature=34, init_weight=True):
        super(Dynamic_conv2d_conditional, self).__init__()
        assert in_planes%groups==0
        self.in_planes = in_planes
        self.out_planes = out_planes
        self.kernel_size = kernel_size
        ...
        self.bias = bias
        self.K = K
        self.attention = attention2d(conditional_dim, ratio, K, temperature)
...
class Multi_Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1, bias=True, K=4,temperature=34, init_weight=True):
        super(Multi_Dynamic_conv2d, self).__init__()
        assert in_planes%groups==0
        self.in_planes = in_planes
        self.out_planes = out_planes
        self.kernel_size = kernel_size
    ...
    def forward(self, x,y):#将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的
        ...
        batch_size, in_planes, height, width = x.size()
        bs, c, h, w = y.size()
        y = y.view(1, -1,h,w).contiguous()# 变化成一个维度进行组卷积
        weight = self.weight.view(self.K, -1)
...
class Dynamic_kv_dim(nn.Module):
    def __init__(self, conditional_dim,in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1, bias=True, K=4,temperature=34, init_weight=True):
        super(Dynamic_kv_dim, self).__init__()
        assert in_planes%groups==0
        self.in_planes = in_planes
        self.out_planes = out_planes
        self.kernel_size = kernel_size
...
class Dynamic_kv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1, bias=True, K=4,temperature=34, init_weight=True):
        super(Dynamic_kv, self).__init__()
        assert in_planes%groups==0
        self.in_planes = in_planes
        self.out_planes = out_planes
        self.kernel_size = kernel_size
