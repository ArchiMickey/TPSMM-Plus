...
import torch
from modules.tpsmm.util import Hourglass, AntiAliasInterpolation2d, make_coordinate_grid, kp2gaussian
from modules.tpsmm.util import to_homogeneous, from_homogeneous, UpBlock2d, TPS
import math
...
class DenseMotionNetwork(nn.Module):
    ...
    def __init__(self, block_expansion, num_blocks, max_features, num_tps, num_channels, 
        ...
        self.maps = nn.Conv2d(hourglass_output_size[-1], num_tps + 1, 1)
        ...
        if multi_mask:
            ...
            for i in range(self.up_nums):
                up.append(UpBlock2d(channel[i], channel[i]//2))
    ...
    def create_deformed_source_image(self, source_image, transformations):
        ...
        use_bf16 = (source_image.dtype == torch.bfloat16) or (transformations.dtype == torch.bfloat16)
        if use_bf16:
            source_repeat = source_repeat.to(torch.float32)
            transformations = transformations.to(torch.float32)
            
        deformed = F.grid_sample(source_repeat, transformations, align_corners=True)
        if use_bf16:
            deformed = deformed.to(torch.bfloat16)
